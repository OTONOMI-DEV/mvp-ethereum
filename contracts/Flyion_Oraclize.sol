...
import "./oraclize/oraclizeAPI.sol"; //NEEDED
import "./FlyionBasics.sol";

contract MSC_Interface { //Using an interface to interact with the MSC
    function updateFromOracle(bytes32 _policyId, bytes32 _flightId, int256 _actualArrDte, uint8 _fltStatus) public; //payable ?
...
event LogCallbackOraclize(bytes32 query, int256 actualArrDte);
...
//Oracle "update" demand -> triggered from the MSC (MSC sends _policyId as parameter).
    ...
    //make sure we have a date returned (not zero)
        int256 actualArrDte = int(parseInt(__actualArrDte)); // use parseint(_actualArrDte) when from oracle, need to importe oraclizeAPI.sol
...
//-- TESTING functions
    ...
    int256 actualArrDte, uint256 fltSts, uint256 calculatedDelay,
